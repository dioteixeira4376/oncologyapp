...
def coletar_dados_paciente():
    ...
    with col1:
        for queixa in queixas[:mid_point]:
            if st.checkbox(queixa, key=f"{queixa}_1"):
    ...
    with col2:
        for queixa in queixas[mid_point:]:
            if st.checkbox(queixa, key=f"{queixa}_2"):
    ...
    for queixa in queixas_gravidade:
        ...
        escolha_intervencao = st.multiselect(
            f"Escolha o tipo de intervenção para {nome_queixa}:",
            ["Intervenção(ões) Interdependente(s)", "Intervenção(ões) Autónoma(s)"],
            key=f"tipo_interv_{nome_queixa}"
        )

        if "Intervenção(ões) Interdependente(s)" in escolha_intervencao:
                intervencao_interdependente = st.multiselect(
                    f"Escolha a(s) intervenção(ões) interdependente(s) para {nome_queixa}:",
                    intervencoes.get(nome_queixa, {}).get("Interdependente", []),
                    key=f"interv_interdep_{nome_queixa}"
                )
                intervencoes_selecionadas.append((nome_queixa, "Interdependente", intervencao_interdependente))

        if "Intervenção(ões) Autónoma(s)" in escolha_intervencao:
            intervencao_autonoma = st.multiselect(
                f"Escolha a(s) intervenção(ões) autónoma(s) para {nome_queixa}:",
                intervencoes.get(nome_queixa, {}).get("Autónoma", []),
                key=f"interv_auton_{nome_queixa}"
            )
            intervencoes_selecionadas.append((nome_queixa, "Autónoma", intervencao_autonoma))
